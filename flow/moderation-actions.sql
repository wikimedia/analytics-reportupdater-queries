...
FROM (
	SELECT DATE_FORMAT(FROM_UNIXTIME((conv(substring(hex(rev_id),1,12),16,10)>>2)/1000),"%Y%m%d%H%i%S") AS report_timestamp,
	       rev_change_type
	FROM flow_revision
	WHERE rev_user_wiki NOT IN ('testwiki', 'test2wiki')
	HAVING report_timestamp >= '{from_timestamp}'
	AND report_timestamp < '{to_timestamp}'
) x;
